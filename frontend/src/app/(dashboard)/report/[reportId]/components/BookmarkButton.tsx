...
export function BookmarkButton(props: BookmarkButtonProps) {
  ...
  const {data: result} = useSWR(`/users/${userId}/bookmarked-reports/${reportId}/exists`, () =>
    ApiService.ins.hasUserBookmarkedReport(userId, reportId), {revalidateOnFocus: true}
  );
  ...
  return (
    <Tooltip title={isBookmarked ? "Remove this report from bookmarked list" : "Bookmark this report"}>
      <IconButton onClick={handleBookmarkReport}>
        {buttonIcon}
      </IconButton>
    </Tooltip>
  );
